...
## 三、CKII 模式映射（Ankora-World 的層級設計哲學）

本專案的層級結構參考 Crusader Kings II（CKII）的封建領地系統：

| CKII 層級 | Ankora 層級 | 實例 |
|-----------|------------|------|
| **Empire** | `empire` | 塞拉菲昂神聖帝國、凱撒里克帝國 |
| **Kingdom / Duchy** | *(隱含於 empire.yaml / properties.liege)* | 格蘭迪斯大公國、莫爾根王國、五大特別市聯盟、護憲封建議會 |
| **County** | `settlement` | 金穗堡、潮音宮、小赫斯領、維特魯斯、雙子門市等 |
| **Holding** | `district` | 城堡區（fortress）、商業區（town）、修道院區（monastery）、農村區（village） |
| **Room** | `room` | 具體房間（王座廳、地牢、密室） |

### County（settlement）與 Kingdom/Duchy 的關係

- **Kingdom/Duchy 不需要獨立 YAML** — 選帝侯領地通過 `empire.yaml` 描述，County 通過 `properties.liege` 或 `properties.suzerain` 標記所屬選帝侯
- **County 是基本地塊單位** — 每個 `settlement` 就是一個 County，擁有獨立的 `id` 和地理邊界
- **Holding 類型由 `settlement_type` 標記** — 但 County 內可包含多種 Holding 類型的 `district`
  - `fortress` = Castle（軍事核心）
  - `town` = City（商業/市民中心）
  - `monastery` = Temple（宗教/知識中心）
  - `village` = 未開發槽位/農業聚落
  - `outpost` = 哨站

### 大城市 = 一 County 多 Holdings

以維特魯斯為例：
- `settlement` = 維特魯斯 County（`settlement_type: town`）
- `district` = 內城區（fortress district：暮光宮）、大教堂區（monastery district：聖鑰院）、商業區（town district）

> **核心原則**：`settlement` 永遠是 County，不是 Holding。命名為「堡」「宮」「莊園」的 County 表示其主要 Holding 為 Castle，但 County 邊界內仍可包含 village/town district。

---

## 四、數據來源優先級（權威性層級）

建立任何地點 YAML 時，子代理**必須**遵循以下優先級，**不得憑空想像**：

| 優先級 | 來源 | 說明 |
|--------|------|------|
| **1（最高）** | `data/*/geographies/*.yaml` | 已結構化的地理層是空間事實的權威。settlement 的位置、地形、氣候、連接必須與 geography 完全一致 |
| **2** | `data/*/empire.yaml` | 帝國層級的政治、歷史、文化框架是 settlement 的社會背景 |
| **3** | `source/ankora-chronicles.md` | 原始敘事文本用於補充細節與氛圍，但**不得與已結構化的 geography/empire 矛盾** |
| **4（最低）** | 推斷 / 創造 | 僅在前三層都無法覆蓋時才允許，且必須標註 `# inferred: true` |

> **核心原則**：geography 層的權威高於 source text。如果 source 中的描述與已建立的 geography YAML 衝突，**以 geography 為準**。
> 
> 例如：wind-isles.yaml 中已定義四座城邦的具體位置與特徵，建立 settlement 時必須直接引用這些已定義的屬性，而非重新發明。
...
| v0.3 | 2026-05-12 | 加入 CKII 模式映射（Empire → County → District → Room）與數據來源優先級
