# Ankora-World 計劃：安納克世界地圖引擎

> **執行後，本計劃將同時寫入 `/workspaces/anak-world/PLAN.md`，確保你隨時可在 Codespaces / CLI / GitHub 上查看。**

---

## 背景與目標

將《安納克大陸誌》（2529 行敘事文本）轉化為結構化的靜態 YAML 世界資料庫。核心原則：靜態優先、層次錨定、AI 無關。

## 現狀

- `README.md` + `source/ankora-chronicles.md`（2529 行原始文本）
- Schema v0.3 + `schema.json`（機器驗證通過）
- AGENTS.md v0.3（子代理協作協議 + 數據來源優先級）
- **66 個 YAML 檔案**已完成：`index.yaml` + 19 geography + 5 empire + 42 settlement/location
- 三個新 geography（Delta Aurum、Campus Cinereus、Septempontis Vallis）+ 戰爭地理學事件已補充
- Connection 網絡一致性修復完成（28 → 3 個遺留問題）
- Git `main` 分支領先 origin 約 15 個提交

---

## 第一階段：骨架與第一個記憶錨點 ✅ 已完成

### 1.1 目錄結構

```
ankora-world/
├── PLAN.md                     # ← 本計劃的可見副本
├── README.md
├── SCHEMA.md                   # YAML 欄位規範（人類可讀）
├── schema.json                 # JSON Schema（機器驗證）
├── source/
│   └── ankora-chronicles.md    # 原始文本移入此處
├── data/                       # 權威結構化資料
│   ├── index.yaml              # 五大區域 + 秘境索引
│   ├── 01-west-seraphion/
│   │   ├── empire.yaml
│   │   └── locations/
│   │       └── vetustapolis/
│   │           └── inner-city/
│   │               └── sacred-key-complex.yaml
│   ├── 02-east-caesaric/
│   ├── 03-north-frost/
│   ├── 04-south-libertania/
│   └── 05-secret-eldara/
├── api/                        # FastAPI 查詢引擎
│   ├── main.py
│   └── requirements.txt
└── site/                       # 靜態前端（預留）
    └── .gitkeep
```

### 1.2 技術骨架（FastAPI + Pydantic + YAML）

- **啟動載入**：遞迴掃描 `data/` 下所有 `.yaml`，以 `id` 為 key 載入記憶體 dict
- **資料驗證**：Pydantic 模型於啟動時驗證所有 YAML，遇錯即拋出明確錯誤
- **四個端點**：
  - `GET /location/{id}` — 單一地點完整 JSON
  - `GET /location/{id}/neighbors` — connections 展開
  - `GET /search?q=` — 跨地點關鍵字搜尋
  - `GET /hierarchy?region=` — 層次樹
- **健康檢查**：`GET /health` 回傳載入統計

### 1.3 第一個示範地點：聖鑰院複合體

從原始文本提取核心資訊，建立 `sacred-key-complex.yaml`，作為所有後續 YAML 的黃金範本。

### 1.4 SCHEMA.md 與 schema.json

定義五級層次（continent → empire → settlement → district → room）與必填欄位，讓未來協作者（人類或 AI）知道如何新增地點。

---

## 第二階段：從 2529 行文本提取「每個細節」 ✅ 已完成

> **核心問題：如何把線性敘事，轉化為結構化的空間座標系？**
> 
> **答案**：三層並行批次處理 + 父代理驗證 + geography 權威優先。

### 2.1 已完成的批次處理

| 批次 | 內容 | 實際產出 | 狀態 |
|------|------|---------|------|
| Batch 1 | Geography 層（山脈、河流、深淵、群島、世界樹） | 14 YAMLs | ✅ |
| Batch 2 | Empire 層（五大帝國） | 5 YAMLs | ✅ |
| Batch 3 | Settlement 層（首都 + 要塞 + 村莊 + 修道院） | 11 YAMLs | ✅ |
| Batch 4 | 一致性修正與歷史補充 | 塔拉薩七海同盟、連接方式修正等 | ✅ |

### 2.2 數據來源優先級（AGENTS.md v0.3）

建立任何地點時必須遵循：
1. **`geography` YAML** > `empire` YAML > `source/ankora-chronicles.md` > 推斷
2. 推斷內容必須標註 `# inferred: true`
3. 若 source 與已結構化的 geography 衝突，**以 geography 為準**

1. **地理實體識別**
   - 遍歷全文，標記所有地名、建築名、區域名
   - 分類層級：帝國（Empire）→ 定居點（Settlement）→ 區域（District）→ 房間（Room）
   - 範例：塞拉菲昂神聖帝國 → 維斯塔波利斯 → 內城區 → 聖鑰院複合體

2. **關係圖譜抽取**
   - 從敘事中識別「A 位於 B 之內」、「C 通往 D」、「E 隸屬於 F」
   - 轉化為 YAML 的 `connections` 與 `parent_id` 機制

3. **感官錨定抽取**
   - 掃描文本中的感官描述詞：氣味（薰香、腐臭、松脂）、光線（燭火、夕照、漆黑）、聲音（禱告、風聲、金屬撞擊）
   - 若原文未明確描述，根據場景性質進行合理推斷並標記 `inferred: true`

4. **歷史事件綁定**
   - 識別文本中的年份、事件名稱，綁定到相關地點的 `events[]`

### 2.3 品質控制機制（已落實）

- ✅ **JSON Schema 驗證**：每次提交前自動驗證 `schema.json`
- ✅ **交叉引用檢查**：`connections[].target` 必須指向真實存在的 `id`
- ✅ **Geography 一致性檢查**：子代理並行驗證 settlement 與 geography 的空間/氣候/連接一致性
- ✅ **Git Diff 審查**：每批次處理後人類審查 diff
- ✅ **數據來源標註**：推斷內容強制標記 `# inferred: true`

### 2.4 感官細節的填充原則

- **原文有則錄**：文本明確寫到的感官描述直接收錄
- **原文無則推**：根據場景性質合理推斷，但必須標記 `inferred: true`
- **不確定則空**：若無法合理推斷，留空 `null` 或 `[]`，不強求填滿

---

## 第三階段：下一輪待執行

### 3.1 District / Room 層級
- 維特魯斯三層空間：inner_city（聖鑰院、暮光宮）、lower_city（百工坊、橋樑市集）、beyond_walls（灰土園、靜默之塔）
- 奧斯堡同心圓：冰壁宮、霜誓廳、破曉號角
- 其他 settlement 的內部區域

### 3.2 勢力層（Faction Layer）
- 霜刃傭兵團（Frostblade Company）
- 銀葉商會（Silverleaf Guild）
- 黑曜石守望者（Obsidian Watch）
- 赭石行會（Ochre Guild）
- 普世教會東西兩派
- 通用欄位：`type`, `headquarters`, `founding_year`, `ideology`, `influence[]`, `relations[]`, `members[]`

### 3.3 FastAPI 查詢引擎
- `GET /location/{id}` — 單一地點完整 JSON
- `GET /location/{id}/neighbors` — connections 展開
- `GET /search?q=` — 跨地點關鍵字搜尋
- `GET /hierarchy?region=` — 層次樹
- `GET /settlement/{id}`（v0.3 路由）

### 3.4 未來擴展（預留）
- **圖譜層**：Kuzu 嵌入式圖資料庫
- **視覺化**：React + Cytoscape.js 靜態網頁
- **AI 敘事層**：MCP / Function Calling 接口

---

## 第四階段：戰爭地理學與世界充足性（待執行）

> **背景**：2026-05-15 審計發現 14 場主要戰爭/衝突中，絕大多數在定居點層級的事件覆蓋嚴重不足。47 個定居點呈現「戰爭失憶症」——無一描寫難民、經濟變化、建築防禦化或戰爭記憶。
>
> **目標**：參考中世紀神聖羅馬帝國（HRE）的封建私戰模式，為世界補充小型戰爭、內部衝突、以及大型戰爭對定居點的漣漪效應。

### 4.0 東方大遷徙路線網絡構想（待補充細節）

> **核心認知**：東遷（第726-729年）不是單一路線的「逃難敘事」，而是**多條路線並行、各有生死、各有策略**的立體遷徙網絡。馬庫斯親王將數萬人分為數路，以分散追兵壓力。

| 路線 | 方向 | 路徑 | 參與者 | 特徵 |
|------|------|------|--------|------|
| **北路（大逃亡）** | 北→東 | 維特魯斯 → 北境凍原 → 繞過怒嘯山脈北端 → 東方平原 | 部分貴族、騎士團殘部、宮廷婦孺 | 最艱險但避開追兵主力；可能遭遇霜生遊牧民襲擾 |
| **中路（怒嘯山脈線）** | 東 | 維特魯斯 → 魯普圖斯隘口 → 怒嘯山脈東麓 → 東方平原 | 馬庫斯親王主力、奧羅·伊格尼斯騎士團、幼帝 | 最直接的陸路，巴爾塔薩追兵必經之處；**節節敗退，背水一戰** |
| **中路偏南（傑米努斯河線）** | 東南→東 | 維特魯斯 → 沿傑米努斯河東岸 → 魯普圖斯隘口南側 → 東方平原 | 工匠、商人、步兵 | 沿河補給，但同樣需穿越隘口；與主力形成掎角之勢 |
| **南路（阿比蘇斯裂谷線）** | 南→東 | 維特魯斯 → 阿比蘇斯裂谷（零散峽谷）→ 深淵之橋 → 東方平原 | 偏師、敢死隊、部分貴族私兵 | 最南端無人區，地勢險峻；**深淵之橋為關鍵爭奪點**——橋前節節敗退，橋上背水一戰 |
| **南路極端（海路繞行）** | 南→海→東 | 維特魯斯 → 南端入海 → 經風息群島外海 → 灰燼商道 → 東部 | 少數擁有船隊的沿海貴族 | 數量極少，繞過所有陸路封鎖；風險在於海盜與風暴 |

**待補充的東遷細節**：
- 各路線的具體事件（年份、戰鬥、損失、轉折點）
- 深淵之橋的爭奪戰細節（人為炸橋？天然岩柱崩塌？）
- 各原帝國貴族如何選擇路線、如何協調、如何失散
- 逃難過程中的日常生活（糧食短缺、疾病、叛逃、內鬥）
- 到達東部後的重建：選址奧斯堡的爭議、資源匱乏、隕鐵短缺、建築技術斷層

### 4.1 高優先級待辦清單

| 批次 | 優先級 | 戰爭/衝突 | 待補充內容 | 預估工作量 |
|------|--------|-----------|-----------|-----------|
| War-1 | **P0** | 百年戰爭（805–825，當下） | 北方前線定居點（霜堡、霜尖塔、奧羅拉堡）補充 events；新建博雷阿利斯王國帝國檔案、里弗邦德定居點 | 高（5-7 檔案） |
| War-2 | **P0** | 第二次白色洪流（451） | 維特魯斯定居點補充陷落事件；沖積平原定居點補充難民/防禦影響 | 中（2-3 檔案） |
| War-3 | **P1** | 塞拉菲昂內部小型戰爭（參考HRE） | 選帝侯私戰、行會叛亂、農民起義；west_seraphion/empire.yaml + 2-3 定居點補充 events | 中（3-4 檔案） |
| War-4 | **P1** | 鐵冠起義（726）定居點記憶 | 維特魯斯補充陷落事件；劍誓池補充東遷記憶；奧斯堡補充東遷因果 | 中（3 檔案） |
| War-5 | **P1** | 大洪水（~0年）定居點記憶 | 維特魯斯、奧斯堡、霍羅托里亞補充洪水起源記憶/祭祀 | 低（3 檔案） |
| War-6 | **P1** | 白冬七年／阿爾比諾克拉西亞內戰（803-810） | north_frost/empire.yaml 補充事件；霍羅托里亞補充「真霜」滲透背景 | 中（2 檔案） |
| War-7 | **P1** | 海峽封鎖戰（~800-825） | 奧斯堡、自由港、塔拉薩補充封鎖相關 events | 中（3 檔案） |
| War-8 | **P2** | 史前戰爭結構化 | 淚痕灘之戰（35）、赫利奧波里斯之圍（78）、鐵籬笆之戰（245）、灰燼的焚燒（395）新建地理/事件 | 低（4-5 檔案） |
| War-9 | **P2** | 裂谷血戰（727） | 新建裂谷隘口地理實體；奧斯堡補充事件引用 | 低（2 檔案） |
| War-10 | **P2** | 邊界禮儀戰（810-825） | 霍羅托里亞補充具體禮儀戰場地與邊境站事件 | 低（1-2 檔案） |

### 4.2 結構性缺口（系統性待解決）

1. **時間斷層**：第801–825年的「當下」衝突覆蓋最差，遠不如第726年前後的東方大遷徙。
2. **定居點失憶症**：47個定居點中，絕大多數 `events` 為空或僅有單一本地事件，無一描述戰爭對日常生活的影響（難民、經濟、防禦、紀念碑）。
3. **描述豐富、事件稀疏**：帝國 YAML 的 `description` 常含大量戰爭背景，但極少結構化為 `events`，導致機器無法直接查詢。
4. **神聖帝國內部衝突空白**：塞拉菲昂的選帝侯制度天然適合封建私戰、行會叛亂、農民起義等小型衝突，但目前幾乎零覆蓋。

---

## 執行順序與預估時間（已執行 + 待執行）

| 階段 | 內容 | 狀態 | 實際時間 |
|------|------|------|---------|
| 1 | 建立目錄結構 + PLAN.md + AGENTS.md | ✅ | ~30 min |
| 2 | 撰寫 SCHEMA.md v0.1 + schema.json | ✅ | ~20 min |
| 3 | Geography 層 14 YAMLs（兩批次） | ✅ | ~2 hrs |
| 4 | Empire 層 5 YAMLs + 驗證 | ✅ | ~1.5 hrs |
| 5 | Schema v0.3（city → settlement + CKII 類型） | ✅ | ~30 min |
| 6 | Settlement 層 11 YAMLs（兩批次 + 修正） | ✅ | ~2.5 hrs |
| 7 | 塔拉薩歷史補充 + 連接方式修正 | ✅ | ~20 min |
| 8 | 三新 Geography + Connection 修復 + 戰爭地理學 | ✅ | ~2 hrs |
| **下一輪** | | | |
| 9 | District / Room 層級 | ⏳ | 待定 |
| 10 | Faction 層 4-6 YAMLs | ⏳ | 待定 |
| 11 | FastAPI 骨架 + 四端點 | ⏳ | 待定 |
| 12 | 戰爭地理學與世界充足性（第四階段） | ⏳ | 待定 |

---

## 一句話指令（下一輪）

> 請選擇執行第三階段任務：建立 District/Room 層級 YAML、建立 Faction 層級 YAML、或建立 FastAPI 查詢引擎。
