...
## 2026-05-12 22:10 Geography 層級擴展：新增 3 個核心地理實體

- **任務**: 回應用戶從「故事設計」與「地理空間設計學」角度補齊 geography 缺口的指示
- **執行方式**: 3 個 subagent 並行生成 + 父代理把關審查
- **新增檔案**: 3 個 geography YAML，全部通過語法驗證與地緣政治一致性審查

### 新增 Geography 清單

| # | 檔案 | ID | 名稱 | 類型 | 區域 | 故事功能 |
|---|------|-----|------|------|------|----------|
| 1 | `lacus-speculum.yaml` | `lacus_speculum` | 斯佩庫拉湖（鏡湖）| 高山湖泊 | 北部/中央交界 | 水源戰略 + 預言/幻象傳說 + 北境邊境緊張 |
| 2 | `silva-argentea.yaml` | `silva_argentea` | 西爾瓦阿根泰亞（銀葉林）| 邊境森林 | 西南部 | 森民隔離緩衝帶 + 人類未知恐懼 + 魔法梯度對比 |
| 3 | `palus-nebulae.yaml` | `palus_nebulae` | 帕魯斯沼澤（迷霧沼澤）| 沼澤/濕地 | 中央/西部 | 帝國內部灰色地帶 + 走私/異端避難所 + 古代遺跡 |
...
| 項目 | 鏡湖 | 銀葉林 | 迷霧沼澤 |
|------|------|--------|----------|
| **山脈位置** | ✅ 怒嘯山脈北麓 + 約恩維德山脈南端交會，合理 | ✅ 塔爾塔魯斯山脈北麓丘陵邊界，合理 | ✅ 中央沖積平原北緣低洼地帶，合理 |
| **水文邏輯** | ✅ 高山湖補給傑米努斯河上游 | ✅ 席爾瓦因河沿北緣流淌 | ✅ 季節性支流終點 + 與主河氾濫連通 |
| **氣候一致性** | ✅ 高海拔冰蝕湖，冬季冰封 | ✅ 受世界樹影響，西暖東冷 | ✅ 沼澤濕熱瘴氣，符合低緯濕地 |

#### 2. 地緣政治審查

| 項目 | 鏡湖 | 銀葉林 | 迷霧沼澤 |
|------|------|--------|----------|
| **帝國歸屬** | ✅ 塞拉菲昂境內，西爾伯弗羅斯特控制 | ✅ 東緣塞拉菲昂，西端艾爾達拉 | ✅ 塞拉菲昂境內，行政模糊 |
| **邊界紅線** | ✅ 非國際邊界，僅邊境緊張 | ✅ 森民完全隔離，無混居 | ✅ 非東西帝國通道，僅內部灰色地帶 |
| **宗教一致性** | ⚠️ 初稿誤寫「太陽神阿努」→ **已修正為普世教會西派** | ✅ 東緣教會西派，西端森民信仰 | ✅ 教會影響力薄弱，異端盛行 |

#### 3. 彼此關聯性審查

- **鏡湖 → 迷霧沼澤**：通過傑米努斯河水系間接連接（鏡湖補給上游，沼澤在中游支流），水文邏輯通順
- **鏡湖 → 銀葉林**：一北一南，無直接連接，符合大陸空間結構
- **銀葉林 → 迷霧沼澤**：一南一北，無直接連接，合理
- **銀葉林與現有 settlement**：與 `radix_specula`（拉迪克斯守望修道院）有 conceptual 連接，強化了邊境監控敘事
- **迷霧沼澤與現有 settlement**：與 `molinaria`（磨坊鎮）和 `ruina_damni`（遺跡村）有 conceptual 連接，形成了格蘭迪斯大公國周邊的完整地理網絡

#### 4. 修正記錄（父代理把關後）

- **鏡湖**：初稿 `religion: 太陽神阿努信仰` → **已修正為「普世教會西派（創世主阿薩）」**，與塞拉菲昂帝國宗教一致
- **銀葉林**：初稿 `controlled_by: 席爾瓦裡翁` → **已修正為「艾爾達拉秘境」**，避免將森民政權過度中央集權化
- **迷霧沼澤**（經兩輪修正）：
  - ⚠️ **第一輪錯誤**：「格蘭迪斯與五大特別市聯盟均宣稱管轄」——五大特別市聯盟無領土野心；「調節上游水閘切斷供水」——中世紀水閘無法完全斷流
  - ✅ **第二輪修正**：
    - 地理定位：從「沖積平原北緣」改為「**沖積平原東部**，傑米努斯河主河道東側的古河道遺跡」
    - 雙子門市位置：從「沼澤南方」改為「**沼澤西方**」（傑米努斯河交匯處）
    - 水權戰爭：從「調節水閘切斷供水」改為「**北岸堰壩優先引水灌溉麥田，導致主河道整體水位銳減**（功能性乾旱）」
    - 宗教：從「異端盛行」改為「**教會與格蘭迪斯世俗權力共治，雙子門市商人公會資助民間水利崇拜**」
  - 帕魯斯沼澤成為雙子門市的**命根**：商人行會祕密開鑿暗渠、建立人力泵站，試圖提取沼澤地下水

### 地理一致性終審

父代理把關後確認以下空間關係閉合：
- 鏡湖（北部高地）→ 經地下溶洞 → 傑米努斯河上游 → 帕魯斯沼澤（中游古河道遺跡）
- 約恩維德河（北向南）→ 沖積平原北側匯入傑米努斯河
- 銀葉林（西南部）→ 東緣塞拉菲昂邊境哨站 → 西端漸變為世界樹森林（森民領地）
- 雙子門市（沖積平原東部交匯處）← 水權壓迫 ← 格蘭迪斯（北岸上游堰壩）

### 新增檔案：安納克大陸 ASCII 地形圖

- `data/map.txt`：80x50 字符的 ASCII 地圖，標註五大帝國、三道山脈、四條大河、主要城市與 geography 實體

### 命名風格
- 全部採用拉丁文：Lacus Speculum, Silva Argentea, Palus Nebulae
...
- [x] `SCHEMA.md` 新增「第五節：定居點粒度分級（Tier System）」
  - Tier 1 `core_hub`：首都級，允許 `settlement.yaml` + `district/` + `room/*.yaml`（維特魯斯模式）
  - Tier 2 `major`：重要城市，單檔為主 + 少數獨立 room（≤5）
  - Tier 3 `minor`：村莊/哨站，單一 YAML，全部內嵌
  - 預設為 Tier 3，升級需審查
  - 欄位使用對照表（`districts`、`contents`、`atmosphere` 等在各級的使用方式）
- [x] `SCHEMA.md`「第六節：新增地點標準流程」更新，加入「確認粒度級別」步驟
- [x] `SCHEMA.md`「第四節：檔案命名與目錄規則」精簡，移除過度具體的目錄結構範例（改放第五節）
- [x] 維特魯斯 `settlement.yaml` 收斂重複內容
  - 聖鑰院複合體、暮光宮、靈魂檢疫塔的描述簡化為摘要，並註明「詳細結構見 room 層 `xxx.yaml`」
  - 避免 settlement 層與 room 層的長篇描述重複

### 現有定居點 Tier 標註（修正：對應 CKII 政治層級）

| Tier | 對應 CKII | 定居點 | 說明 |
|------|-----------|--------|------|
| **Tier 1** | **Empire Capital** | 維特魯斯、奧斯堡、霍羅托里亞、塔拉薩、席爾瓦裡翁 | 五大政治實體的首都/核心。維特魯斯已完整展開，其餘維持單檔，未來可視敘事需要展開 |
| **Tier 2** | **Duchy/Kingdom Capital** | 金穗堡、潮音宮、霜尖塔要塞、雙子門市、焰紋市、鹽白城、阿申斯塔特、菲惹港、昂科拉、艾歐利斯 | 選帝侯領地首都、重要自治城邦。單檔為主，未來可視需要開少量獨立 room（≤5） |
| **Tier 3** | **普通 County** | 赤土鎮、小赫斯領、珍珠灣漁村、流泉修道院、灰土園墓窖、永火燈塔修道院、阿什福德塔堡、巴羅瑪奇堡、堤壩遺跡哨站、格蘭迪斯-阿申斯塔特邊境哨站、北岸黑麥村、夕照畿農奴村、南方海岸鹽田、私掠艦隊錨地、霜望要塞、荊棘維克莊園 | 普通城鎮、村莊、哨站、小修道院、邊境堡壘。單一 YAML，全部內嵌 |

### 設計決策（修正後）

- **Tier 由政治層級決定，非主觀感受**：帝國首都 = Tier 1，選帝侯首都 = Tier 2，普通 County = Tier 3
- **不強制降級**：維特魯斯保持 Tier 1 完整展開
- **Tier 2 不濫用**：選帝侯級定居點預設單檔，僅當有超過 3 個值得獨立描述的 room 時才展開
- **核心原則**：`settlement.yaml` 的 `contents` 在 Tier 1 中應保持摘要性，詳細描述交給獨立 `room` 檔案；Tier 2/3 的 `contents` 是主要描述載體
